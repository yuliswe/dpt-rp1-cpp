...
{
  assert(
    ! m_private_key_path.empty()
      && "don't forget to set private key"
  );
  assert(
    ! m_client_id_path.empty()
      && "don't forget to set client id"
  );
  try {
  m_cookies.clear();
  string client_id;
...
string HttpSigner::sign(string nonce)
{
  ...
  error = EVP_DigestSignInit(
    mdctx,
    &pkey_ctx,
    md,
    NULL,
  evp_pkey);
  error = EVP_DigestSignUpdate(
    mdctx,
    nonce.c_str(),
    nonce.length()
  );
...
{
  auto request = nativeformat::http::createRequest(
  baseUrl() + url,
  ...
  request->headerMap()["Cookie"] =
    "Credentials=" + m_cookies.at("Credentials");
  }
  return static_pointer_cast<DptRequest>(request);
}

shared_ptr<DptResponse> Dpt::sendRequest(
...
) const
{
  ...
  logger()
    << "request: "
    << request->serialise() << endl;
  #endif
  #if DEBUG_REQUEST_BODY
  logger()
    << "request body: "
    << request->body().substr(0,1000)
    << endl;
  #endif
  static auto client =
  nativeformat::http::createClient(
    nativeformat::http::standardCacheLocation(),
    "NFHTTP-" + nativeformat::http::version()
  );
  auto resp = static_pointer_cast<DptResponse>(
  client->performRequestSynchronously(request)
  );
  #if DEBUG_REQUEST
  ...
  logger()
    ...
    << endl;
  logger()
    << "Request responsible for the error was: "
    << request->serialise()
    << endl;
...
void Dpt::syncTime() const
{
  ...
  ostringstream os;
  // "2019-05-23T01:07:03Z";
  os << std::put_time(std::gmtime(&now), "%FT%TZ");
  /* set time on DPT-RP1 */
  Json js;
  js.put("value", os.str());
...
void Dpt::updateDptTree()
{
  ...
  string parent_path = path(
    val.get<string>("entry_path")
  ).parent_path().string();
  if (m_dpt_path_nodes.find(parent_path)
      == m_dpt_path_nodes.end())
  {
...
void Dpt::moveBetweenDpt(path const& source, path const& dest)
{
  ...
  logger()
    ...
    << source << " ~> " << dest << endl;
  #endif
  shared_ptr<DNode const> source_node =
    m_dpt_path_nodes[source.string()];
  shared_ptr<DNode const> dest_parent_node =
    m_dpt_path_nodes[dest.parent_path().string()];
  if (source_node->isDir()) {
    Json js;
    js.put("parent_folder_id", dest_parent_node->id());
    js.put("folder_name", dest.filename().string());
    auto resp = sendJson("PUT",
      "/folder2/" + source_node->id(),
      js
    );
  } else {
    Json js;
...
void Dpt::copyBetweenDpt(path const& source, path const& dest)
{
  ...
  while (! que.empty()) {
    ...
    shared_ptr<DNode const> dest_parent_node =
      m_dpt_path_nodes[n_dest_path.parent_path().string()];
    if (n->isDir()) {
...
void Dpt::computeSyncFiles()
{
  ...
  */
  ...
  {
    if (
      matched_local_nodes.find(local_node)
        == matched_local_nodes.end()
    )
    {
    ...
    if (db_row.empty()) {
      ...
      if (db_row[LocalRev] == local->rev())
      {
        ...
        if (db_row[DptRev] == dpt->rev())
        {
          ...
          if (local->relPath() != dpt->relPath()) {
            ...
            /* need to sync path */
            if (
              local->lastModifiedTime() > dpt->lastModifiedTime()
            )
            {
              #if DEBUG_CONFLICT
                logger()
                  << "local version have later last-modified."
                  << " will move dpt file" << endl;
              #endif
              m_prepared_dpt_move.push_back(make_pair(dpt, local));
            } else {
              #if DEBUG_CONFLICT
                logger() << "dpt version have later last-modified. "
                  << " will move local file" << endl;
              #endif
        ...
        else
        {
          #if DEBUG_CONFLICT
            logger()
              << "dpt version has been modified and is "
              << "the newer version" << endl;
          #endif
          /* dpt file is newer */
          m_prepared_overwrite_from_dpt.push_back(dpt);
          if (local->relPath() != dpt->relPath()) {
            #if DEBUG_CONFLICT
              logger()
                << "two files have different paths."
                << " local version will be deleted." << endl;
            #endif
            m_prepared_local_delete.push_back(local);
          }
        }
      ...
      else
      {
        ...
        if (db_row[DptRev] == dpt->rev())
        {
          ...
          if (local->relPath() != dpt->relPath()) {
            #if DEBUG_CONFLICT
              logger() << "two files have different paths."
                << " dpt version will be deleted." << endl;
            #endif
            m_prepared_dpt_delete.push_back(dpt);
          }
        }
        else
        {
          #if DEBUG_CONFLICT
            logger()
              << "dpt version has been modified. conflict!"
              << endl;
          #endif
          ...
          if (local->lastModifiedTime() > dpt->lastModifiedTime())
          {
            #if DEBUG_CONFLICT
              logger()
                << "local version have later last-modified date."
                << " use local file and delete dpt file"
                << endl;
            #endif
            /* local is newer */
            m_prepared_overwrite_to_dpt.push_back(local);
            ...
            #if DEBUG_CONFLICT
              logger()
                << "dpt version have later last-modified date."
                << " use dpt file and delete local file"
                << endl;
            #endif
            /* dpt is newer */
            m_prepared_overwrite_from_dpt.push_back(dpt);
...
)
{
  ...
  if (local->isDir()) {
    ...
    if (! (only_local.empty() && only_dpt.empty())) {
      logger() << "only_local: ";
      for (auto const& i : only_local) {
        logger() << "(" << i->filename() << ") ";
      }
      logger() << endl;
      logger() << "only_dpt: ";
      for (auto const& i : only_dpt) {
        logger() << "(" << i->filename() << ") ";
        }
      logger() << endl;
      logger() << "both: ";
      for (auto const& i : both) {
        logger() << "(" << i.first->filename() << ") ";
      }
      logger() << endl;
...
void Dpt::computeSyncFilesInNode(
  shared_ptr<DNode const> local,
  shared_ptr<DNode const> dpt
)
{
  ...
  m_local_only_nodes.insert(
    m_local_only_nodes.end(),
    only_local.begin(),
    only_local.end()
  );
  m_dpt_only_nodes.insert(
    m_dpt_only_nodes.end(),
    only_dpt.begin(),
    only_dpt.end()
  );
  /* for dirs & files that exist at both ends */
...
void Dpt::overwriteFromDpt(path const& source, path const& dest)
{
  ...
  while (! que.empty()) {
    ...
    if (n->isDir()) {
      ...
      /* if local file exists, then bisect for the first byte two
        files diverse, and only download the different part */
      shared_ptr<LNode> local_node;
      if (
        m_local_path_nodes.find(n_dest_path.string())
          == m_local_path_nodes.end()
      )
      {
        ofstream of(
          n_dest_path.string(),
      ...
      /* Todo: read whole file into a buffer instead of writing in
      place */
      fstream iof(
        n_dest_path.string(),
        ios_base::binary|ios_base::out|ios_base::in
      );
      size_t const local_filesize = readLocalFilesize(iof);
      size_t offset = 0;
      if (! n->isNote()) {
        // get where the difference starts
        // doesn't quite work with notes
        offset = bisectDptFileBytes(n, iof);
      }
      #if DEBUG_FILE_IO
        logger()
          << "writing file (offset="
...
shared_ptr<vector<uint8_t>> Dpt::readDptFileBytes(
  shared_ptr<DNode const> n,
  size_t offset,
  size_t size
) const
{
  ...
  request->headerMap()["Range"] =
    "bytes=" + to_string(offset) + "-" + to_string(offset+size-1);
  auto response = sendRequest(request);
  size_t bytes;
...
bool dpt::syncable(path const& path)
{
  ...
  return is_directory(path)
    || (is_regular_file(path) && path.extension() == ".pdf");
}
...
{
  /* Sometimes DPT-RP1 reports wrong filesize initially. This looks
    like a bug on DPT-RP1. We can force DPT-RP1 to update the file by
    reading a random byte of the file */
...
shared_ptr<vector<uint8_t>> dpt::readLocalFileBytes(
  istream& inf,
  size_t offset,
  size_t size
)
...
void Dpt::overwriteToDpt(path const& source, path const& dest) {
  ...
  while (! que.empty()) {
    ...
    if (is_directory(n)) {
      ...
      js.put(
        "parent_folder_id",
        m_dpt_path_nodes[n_dest_path.parent_path().string()]->id()
      );
      js.put(
        "folder_name",
      ...
      // if file exists
      if (
        m_dpt_path_nodes.find(n_dest_path.string())
        == m_dpt_path_nodes.end()
      )
      {
        #if DEBUG_FILE_IO
          logger()
            << "creating file on dpt: " << n_dest_path << endl;
        #endif
        Json js;
        js.put(
          "parent_folder_id",
...
) const
{
  ...
  auto request = httpRequest(
    "/documents/" + node->id()
    + "/file?offset_bytes=" + to_string(offset)
    + "&total_bytes=" + to_string(total)
    + "&last_byte=" + to_string(offset + bytes->size())
  );
  request->setMethod("PUT");
  request->headerMap()["Content-Type"]
    = "multipart/form-data; boundary=DptAirBound";
  /* build header */
...
void Dpt::safeSyncAllFiles(DryRunFlag dryrun)
{
  ...
  try {
    ...
    logger() << "An error happend during syncing, "
      << "changes will be reverted." << endl;
...
void Dpt::setMessager(
  std::function<void(string const&)> me
) noexcept
{
  m_messager = me;
}
...
void Dpt::dptQuickUploadAndOpen(path const& local)
{
  try {
    ...
    updateDptTree();
    if (
      m_dpt_path_nodes.find("Document/Received")
        == m_dpt_path_nodes.end()
    )
    {
      Json js;
...
vector<shared_ptr<GitCommit>> Dpt::listGitCommits(
  size_t limit
) const
{
  size_t lim = min(limit, m_git_commits.size());
  return vector<shared_ptr<GitCommit>>(
    m_git_commits.begin(),
    m_git_commits.begin() + lim
  );
}
